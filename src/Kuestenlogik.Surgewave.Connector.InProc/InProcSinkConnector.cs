...
/// <summary>
/// Sink connector that sends messages to in-process channels, named pipes, or shared memory.
/// </summary>
public sealed class InProcSinkConnector : SinkConnector
{
    public override string Version => "1.0.0";

    public override Type TaskClass => typeof(InProcSinkTask);

    public override ConfigDef Config => new ConfigDef()
        .Define(InProcConnectorConfig.Topics, ConfigType.String, Importance.High, "Source topics to consume", EditorHint.Topic)
    ...
    {
        if (!config.TryGetValue(InProcConnectorConfig.Topics, out var topics) || string.IsNullOrEmpty(topics))
            throw new ArgumentException($"Missing required config: {InProcConnectorConfig.Topics}");

        _topics = topics.Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries).ToList();
    ...
    public override IReadOnlyList<IDictionary<string, string>> TaskConfigs(int maxTasks)
    {
        ...
        {
            [InProcConnectorConfig.Topics] = string.Join(",", _topics),
