...
/// <summary>
/// Tests for the X-Hub-Signature-256 check of the Facebook webhook. Without it anyone who can
/// reach the listener port can inject page events, so a configured app secret has to reject
/// every delivery it cannot verify.
/// </summary>
public class FacebookWebhookSignatureTests
{
    ...
    public void WithoutAnAppSecret_VerificationIsSkipped()
    {
        ...
        Assert.True(FacebookSourceTask.IsSignatureValid(secret: null, header: null, Body));
    }

    [Fact]
    public void AcceptsADeliverySignedWithTheAppSecret()
    {
        Assert.True(FacebookSourceTask.IsSignatureValid(AppSecret, Sign(AppSecret, Body), Body));
    }
    ...
    public void RejectsADeliverySignedWithAnotherSecret()
    {
        ...
        Assert.False(FacebookSourceTask.IsSignatureValid(AppSecret, forged, Body));
    }
    ...
    public void RejectsADeliveryWhoseBodyWasTamperedWith()
    {
        ...
        Assert.False(FacebookSourceTask.IsSignatureValid(AppSecret, header, tampered));
    }
    ...
    {
        Assert.False(FacebookSourceTask.IsSignatureValid(AppSecret, header, Body));
    }
